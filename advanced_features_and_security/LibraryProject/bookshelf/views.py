...
def edit_books(request, books_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('books_detail', books_id=books.id)  # Redirect to the books detail view
    else:
...
def delete_books(request, books_id):
    ...
    if request.method == 'POST':
        books.delete()
        return redirect('books_list')  # Redirect to the books list view after deletion
